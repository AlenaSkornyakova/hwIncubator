...
describe('Videos API', () => {
  ...
  it('should create a new entity with correct input data and return it', async () => {
    ...
    await request(app).get(`${routerPath.videos}`).expect(HTTP_STATUSES.OK_200, [createdEntity]);
  });
  ...
  it('should return entity by existing id', async () => {
    ...
    const response = await request(app)
      .get(`${routerPath.videos}/${createdEntity.id}`)
      .expect(HTTP_STATUSES.OK_200);
    expect(response.body).toEqual(createdEntity);
  });

  it('should return 404 for non-existing id', async () => {
    await request(app).get(`${routerPath.videos}/9999`).expect(HTTP_STATUSES.NOT_FOUND_404);
  });
  ...
  it('should update the entity by id with correct input data', async () => {
    const updatedData: VideoUpdateInputDto = {
      ...
      availableResolutions: [ VideoResolution.P360, VideoResolution.P480],
    ...
    await request(app)
      .put(`${routerPath.videos}/${createdEntity.id}`)
      .send(updatedData)
  ...
  it('should delete the entity by id', async () => {
    const { createdEntity } = await videosTestManager.createVideo(createdData);
    await request(app).delete(`${routerPath.videos}/${createdEntity.id}`).expect(HTTP_STATUSES.NO_CONTENT_204);
    await request(app).get(`${routerPath.videos}/${createdEntity.id}`).expect(HTTP_STATUSES.NOT_FOUND_404);
  });
});
